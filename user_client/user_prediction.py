...
def load_RNN(code,company):
    try:
        ...
        predict_price_RNN = predict.RNN.iloc[-1]
...
def load_CNN(code,company):
    try:
        ...
        predict_price_CNN = predict.CNN.iloc[-1]


        prediction_1 = str(predict_date) + ' 종가 : '+ str(real_price)
        prediction_1 += '\n----------------------------------'
        prediction_1+= '\n다음 거래일 종가 증감률 예측 : '+str(predict_price_CNN)


        if len(predict) > 1:
            predict_price_CNN_1 = predict.CNN.iloc[-2]
            real_price_1 = stock.close.iloc[-2]
            real_price_percent = round((real_price_1 / real_price - 1) * 100, 3)
            prediction_1 += '\n----------------------------------'
            prediction_1 += '\n이전 거래일 종가 증감률 예측 : ' + str(predict_price_CNN_1)
            prediction_1 += '\n----------------------------------'
            prediction_1 += '\n이전 거래일 실제 종가 증감률 : ' + str(real_price_percent)+'%'
